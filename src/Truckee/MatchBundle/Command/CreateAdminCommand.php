...
class CreateAdminCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('match:admin:create')
            ->setDescription('Create an admin user.')
            ->setHelp(<<<EOT
The <info>match:admin:create</info> command creates a user:

  <info>php app/console match:admin:create</info>
...
You can alternatively specify the username, email, first name, last name, password, and type as arguments:
  ...
  <info>php app/console match:admin:create bborko borko@bogus.info Benny Borko mypassword staff</info>

You can create an inactive user (will not be able to log in):

  <info>php app/console match:admin:create bborko --inactive</info>
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $admin = $em->getRepository('TruckeeMatchBundle:Person')->findOneBy(['username' => $username]);
