...
class OpportunityRepository extends EntityRepository
{
    ...
    public function doFocusSkillSearch($data)
    {
        ...
        if (!$focusesExist && !$skillsExist) {
            ...
            foreach ($opportunities as $key => $opp) {
                if ($focusesExist) {
                    $org = $em->getRepository("AppBundle:Organization")->find($opp['orgId']);
                ...
                if ($skillsExist) {
                    $opportunity = $em->getRepository("AppBundle:Opportunity")->find($opp['id']);
    ...
    public function noEmails()
    {
        return $this->getEntityManager()
                        ->createQuery(
                                'SELECT p FROM AppBundle:Opportunity p '
                                . 'LEFT JOIN AppBundle:AdminOutbox a '
    ...
    public function expiringOppsSent()
    {
        ...
        $qb->select('o')
            ->from('AppBundle:Opportunity', 'o')
            ->join('AppBundle:AdminOutbox', 'a', 'WITH', $qb->expr()->eq('a.oppId', 'o'))
    ...
    public function expiringOppsNotSent()
    {
        ...
        $qb->select('o')
            ->from('AppBundle:Opportunity', 'o')
            ->leftJoin('AppBundle:AdminOutbox', 'a', 'WITH', $qb->expr()->eq('a.oppId', 'o'))
