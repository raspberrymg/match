...
//src\AppBundle\Tools\AdminMailer.php

namespace AppBundle\Tools;

use AppBundle\Entity\AdminOutbox;
...
class AdminMailer
{
    ...
    public function sendNewOrganization($organization)
    {
        $message = \Swift_Message::newInstance()
            ...
            ->setBody(
            $this->twig->render(
                'AppBundle:Email:new_organization.html.twig',
                array(
    ...
    public function sendOppInterestEmail($to, $from, $subject, $content)
    {
        ...
        if (!empty($recipient)) {
            $message        = \Swift_Message::newInstance()
                ...
                ->setBody(
                $this->twig->render(
                    "AppBundle:Default:onlineOppEmailContent.html.twig",
                    array(
                    'content' => $content,
                    'recipient' => $recipient,
                    ), 'text/html'
    ...
    {
        $em         = $this->em;
        $admins     = $em->getRepository("AppBundle:Admin")->findBy(['locked' => false]);
        $adminEmail = [];
